namespace Ticketmaster.Discovery.Tests.V2.Extensions
{
    ...
    public static class EventsClientExtensions
    {
        ...
        )
        {
            ...
            SearchEventsResponse result = await client.SearchEventsAsync(originalRequest);
        ...
        private static void NextPage(this SearchEventsRequest originalRequest, SearchEventsResponse response)
        {
            ...
            if (response.Page.Number >= response.Page.TotalPages) return;
            ...
            if (response.Page.Size * pageNumber < ApiConstraints.MaxPagingDepth) return;

            var pageSize = response.Page.Size;
            while (pageSize * pageNumber >= ApiConstraints.MaxPagingDepth) pageSize--;
