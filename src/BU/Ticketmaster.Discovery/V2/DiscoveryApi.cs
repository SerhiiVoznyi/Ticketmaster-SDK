...
namespace Ticketmaster.Discovery
{
    ...
    public class DiscoveryApi : IDiscoveryClient, IDisposable
    {
        ...
        public IApiClient Configure(IClientConfig config)
        {
            ...
            _attractions =
                new Lazy<IAttractionsClient>(() => new AttractionsClient(new RestClient(config.ApiRootUrl), config));
            _classifications = new Lazy<IClassificationsClient>(() =>
                new ClassificationsClient(new RestClient(config.ApiRootUrl), config));
            _suggestions =
                new Lazy<ISuggestionsClient>(() => new SuggestionsClient(new RestClient(config.ApiRootUrl), config));
